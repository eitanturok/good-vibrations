...
# ***** running it *****
#
# 16 runs, sequential on the single GPU: 4 faces x 4 arms. None of these column sets matches an
# arm of scripts/laser_faces.sh (that sweep used the 5-column faces), so all 16 are new work and
# no baseline here can be reused from it.
#
...
# 16 separate builds, not 4, and the `all` builds are the biggest at 64 lasers. Each is a one-off
# ~1 min for this capture.
