"""Array -> PNG. Only the laser x freq heatmap needs this: 100x1235 cells is ~1.5 MB as
JSON but ~40 KB as a PNG the browser decodes in hardware. Everything smaller ships as JSON.

Palette is viz/render.py's sequential blue, so both dashboards read as one product.
"""
...
def heat(v, lo, hi):
    ...
    img = Image.fromarray(LUT[(t * 255).astype(np.uint8)])
    b = io.BytesIO()
    img.save(b, "PNG")
    return b.getvalue()
