...
def phase_sync(fft, laser_idx=0, xy_idx=0, eps=1e-20):
    ...
    fft_synced = (
        fft * phase[None, :, None]
    )  # (L, F, 2) — same rotation applied to x and y
    assert all(abs(v.item()) < 1e-5 for v in fft_synced[laser_idx, :, xy_idx].imag), (
        "phase sync failed: reference laser x-channel is not real after rotation"
    )
...
def global_magnitude(fft, eps=1e-20):
    return fft / (fft.abs().std() + eps)
...
def local_magnitude(fft, eps=1e-20):
    return fft / (fft.abs().std(dim=1, keepdim=True) + eps)


# Both phase sync and global magnitude normalization
def global_magnitude_and_phase_sync(fft, eps=1e-20):
    return global_magnitude(phase_sync(fft, eps=eps), eps=eps)
...
def local_magnitude_and_phase_sync(fft, eps=1e-20):
    return local_magnitude(phase_sync(fft, eps=eps), eps=eps)
...
class VibrationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if DISCRETIZED_MASK:
            ...
            ).squeeze()  # (H, W) -> (disc_mask_h, disc_mask_w)

        def round_to_floor(x, n):
            return min(int(x / n), n - 1)

        H, W = mask.shape[-2], mask.shape[-1]
        floor_x, floor_y = (
            round_to_floor(row["x_position"], W / self.floor_cols),
            round_to_floor(row["y_position"], H / self.floor_rows),
        )
...
def make_collate(
    ...
    normalize_fn = {
        "global-magnitude": global_magnitude,
        "local-magnitude": local_magnitude,
        "phase-sync": phase_sync,
        "global-magnitude-phase-sync": global_magnitude_and_phase_sync,
        "local-magnitude-phase-sync": local_magnitude_and_phase_sync,
    }.get(normalize, None)
...
def get_dataloaders(
    ...
    train_collate_fn, test_collate_fn = (
        ...
        make_collate(patch_size, augment=False, normalize=normalize),
    )
    ...
    fft_patches = test_collate_fn([dataset[0]])[0][
        0
    ]  # (n_lasers, n_patches, 2, patch_size)
    ...
    data_info = {
        ...
        "disc_mask_h": disc_mask_h,
        "disc_mask_w": disc_mask_w,
        "num_lasers": fft_patches.shape[0],
        "n_freqs": fft_patches.shape[1] * patch_size,
        "n_patches": fft_patches.shape[1],
    }
...
def soft_dice_fn(logits, targets, axis=None):
    ...
    if axis is not None:
        probs, targets = probs.mean(dim=axis), targets.mean(dim=axis)
    spatial_dims = tuple(
        range(1, probs.ndim)
    )  # (-2, -1) for dice over entire mask or (-1,) for dice on just x,y profiles
...
def weighted_cross_entropy_fn(mask_logits, target):
    ...
    pos_frac = (
        target.sum(dim=(-2, -1)).mean() / n_pixels
    )  # mean positive fraction across batch
    neg_frac = 1.0 - pos_frac
    pos_weight = (neg_frac / pos_frac.clamp(min=1e-6)).clamp(max=100.0)
    return F.binary_cross_entropy_with_logits(
        mask_logits, target, pos_weight=pos_weight
    )
...
def boundary_loss_fn(mask_logits, target, kernel_size=3):
    ...
    dilated = F.max_pool2d(
        target.unsqueeze(1), kernel_size, stride=1, padding=pad
    ).squeeze(1)
    eroded = -F.max_pool2d(
        -target.unsqueeze(1), kernel_size, stride=1, padding=pad
    ).squeeze(1)
...
LOSSES = {
    "ce": F.cross_entropy,
    "wce": weighted_cross_entropy_fn,
    "focal": focal_loss_fn,
    "asym_focal": asymmetric_focal_loss_fn,
    "mse": mse_loss_fn,
    "dice": soft_dice_fn,
    "tversky": tversky_loss_fn,
    "boundary": boundary_loss_fn,
}
...
class MLPDecoder(nn.Module):
    ...
    def __init__(self, d_model, out_h, out_w):
        ...
        self.net = nn.Sequential(
            nn.Linear(d_model, 256), nn.ReLU(), nn.Linear(256, out_h * out_w)
        )
...
class CNNDecoder(nn.Module):
    ...
    def __init__(self, d_model, out_h, out_w):
        ...
        self.up2 = nn.Sequential(
            nn.ConvTranspose2d(
                256, 128, kernel_size=(4, 3), stride=(2, 1), padding=(1, 1)
            ),
            nn.GroupNorm(8, 128),
            nn.GELU(),
...
class CrossAttnDecoder(nn.Module):
                ...
                nn.ModuleDict(
                    {
                        "cross_attn": nn.MultiheadAttention(
                            d_model, num_heads=4, batch_first=True, dropout=0.1
                        ),
    ...
    def forward(self, cls, laser_feats):
        ...
        q = apply_rope(
            self.mask_queries.expand(B, -1, -1), self.mask_pos
        )  # (B, H*W, D)
        for layer in self.layers:
            q = layer["cross_norm"](
                q + layer["cross_attn"](q, laser_feats, laser_feats)[0]
            )
...
class PoolDecoder(nn.Module):
    ...
    def forward(self, cls, laser_feats):
        ...
        pool25 = (
            F.softmax(self.pool_25(laser_feats), dim=1).transpose(1, 2) @ laser_feats
        ).mean(1)  # (B, D)
        pool4 = (
            F.softmax(self.pool_4(laser_feats), dim=1).transpose(1, 2) @ laser_feats
        ).mean(1)  # (B, D)
...
def build_decoder(decoder, d_model, out_h, out_w, cross_attn_layers=4):
    if decoder == "mlp":
        return MLPDecoder(d_model, out_h, out_w)
    elif decoder == "cnn":
        return CNNDecoder(d_model, out_h, out_w)
    elif decoder == "cross_attn":
        return CrossAttnDecoder(d_model, out_h, out_w, cross_attn_layers)
    elif decoder == "pool":
        return PoolDecoder(d_model, out_h, out_w)
    else:
...
def precompute_freqs_cis_2d(
    dim: int, h: int, w: int, theta: float = 10000.0
) -> torch.Tensor:
    freqs_h, freqs_w = (
        precompute_freqs_cis(dim // 2, h, theta),
        precompute_freqs_cis(dim // 2, w, theta),
    )
    freqs_h, freqs_w = (
        freqs_h.reshape(h, 1, -1).repeat(1, w, 1),
        freqs_w.reshape(1, w, -1).repeat(h, 1, 1),
    )
...
class LearnablePositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        return x + self.embed(torch.arange(x.shape[1], device=x.device).unsqueeze(0))


class PointTransformer(nn.Module):
    def __init__(
        self, patch_size, d_model, num_heads, num_layers, signal_length, signal_is
    ):
        super().__init__()
        ...
        if ROPE:
            self.register_buffer(
                "freqs_cis", precompute_freqs_cis(d_model, signal_length // patch_size)
            )
        else:
            self.freq_pos_embd = LearnablePositionalEncoding(
                signal_length // patch_size, d_model
            )
        self.raw_to_tokens = {
            "magnitude": lambda t: t.abs(),
            "complex": lambda t: torch.cat([t.real, t.imag], dim=-1),
            "mag_phase": lambda t: torch.cat([t.abs(), t.angle()], dim=-1),
        }[signal_is]

    def forward(self, x):
        # x.shape = (B_L,P,C,_PS) = (batch_size * n_lasers, n_patches, n_coords, patch_size)
        B_L, P, _, _ = x.shape
        x = self.raw_to_tokens(
            x
        ).float()  # (B_L,P,C,_PS) -> (B_L,P,C,PS) where PS=_PS or 2*_PS
        x = self.embed(x.reshape(B_L, P, -1))  # (B_L,P,C,PS) -> (B_L,P,D)
        x = (
            apply_rope(x, self.freqs_cis.to(x.device))
            if ROPE
            else self.freq_pos_embd(x)
        )  # (B_L,P,D)
        x = torch.cat(
            (self.cls_token.expand(B_L, -1, -1).to(x.device), x), dim=1
        )  # (B_L,P+1,D)
...
class SignalTransformer(ComposerModel):
    def __init__(
        ...
        self.floor_cols, self.floor_rows = (
            data_info["floor_cols"],
            data_info["floor_rows"],
        )
        laser_rows, laser_cols = (
            int(math.sqrt(data_info["num_lasers"])),
            int(math.sqrt(data_info["num_lasers"])),
        )
        self.out_h, self.out_w = (
            (data_info["disc_mask_h"], data_info["disc_mask_w"])
            if DISCRETIZED_MASK
            else (data_info["mask_h"], data_info["mask_w"])
        )
        self.pnt_trans = PointTransformer(
            patch_size,
            d_model,
            pnt_num_heads,
            pnt_num_layers,
        ...
        self.seq_trans = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(
                d_model=d_model, nhead=seq_num_heads, batch_first=True
            ),
        ...
        if ROPE:
            self.register_buffer(
                "freqs_laser", precompute_freqs_cis_2d(d_model, laser_rows, laser_cols)
            )
        else:
            self.laser_pos_embd = LearnablePositionalEncoding(
                laser_rows * laser_cols, d_model
            )
        ...
        self.train_metrics, self.val_metrics = (
            create_metrics(self.floor_cols, self.floor_rows),
            create_metrics(self.floor_cols, self.floor_rows),
        )
    ...
    def forward(self, batch):
        ...
        x = (
            apply_rope(x, self.freqs_laser.to(x.device))
            if ROPE
            else self.laser_pos_embd(x)
        )  # (B,L,D) -> (B,L,D)
        x = torch.cat(
...
def get_parser():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "local-magnitude",
            "phase-sync",
            "global-magnitude-phase-sync",
            "local-magnitude-phase-sync",
        ],
    )
    parser.add_argument(
        "--loss",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[1],
        help="List of n_objects values to include, e.g. --n-objects 1 2",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="mlp",
        choices=["mlp", "cnn", "cross_attn", "pool"],
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="mse",
        help="Eval metric key to gate best-checkpoint saving on",
    )
    parser.add_argument(
        "--best-metric-higher-is-better",
        action="store_true",
        default=False,
        help="Set if larger metric values are better (e.g. accuracy)",
    )
    parser.add_argument(
        "--checkpoint-interval",
        type=str,
...
def train(**kwargs):
    ...
    SORD, MASK, POSITION, ROPE, DISCRETIZED_MASK, FOCAL, AUGMENT, HARD_MASK = (
        args.sord,
        args.mask_loss,
        args.position_loss,
        args.rope,
        args.discretized_mask,
        args.focal,
        args.augment,
        args.hard_mask,
    )
    ...
    train_loader, test_loader, data_info = get_dataloaders(
        args.data_dir,
        args.patch_size,
        args.disc_mask_h,
        args.disc_mask_w,
    ...
    config = {
        ...
        **data_info,
        ...
        "seed": args.seed,
        "signal_is": args.signal_is,
        "d_model": args.d_model,
        "pnt_num_heads": args.pnt_num_heads,
        "seq_num_heads": args.seq_num_heads,
        "pnt_num_layers": args.pnt_num_layers,
    ...
    best_saver = BestMetricCheckpointSaver(
        ...
        save_path=best_checkpoint_path(run_id),
    ...
    trainer = Trainer(
        ...
        autoresume=True,
        callbacks=[mask_viz, best_saver, hf_uploader, RuntimeEstimator(), SpeedMonitor(1), OOMObserver(), NaNMonitor()],
    )
