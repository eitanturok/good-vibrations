...
class MaskVisualizationCallback(Callback):
    def __init__(self, n_samples=4, save_dir="visualizations", train_viz_interval=10, thresholds=[],
                 pred_save_path=None, run_id=None):
        self.n_samples = n_samples
        self.save_dir = save_dir
        self.train_viz_interval = train_viz_interval
        self.thresholds = list(thresholds)
        self.pred_save_path = pred_save_path  # HF repo ID, e.g. "eturok-weizmann/good-vibrations"
        self.run_id = run_id
    ...
    def epoch_end(self, state, logger):
        if state.timestamp.epoch.value % self.train_viz_interval == 0 and self._last_train_batch is not None:
            ...
            self._save_predictions([self._batch_to_pred(batch, outputs)], state, "train")
    ...
    def eval_end(self, state, logger):
        if not self._eval_preds: return
    ...
    def _save_predictions(self, preds_list, state, split):
        if not self.pred_save_path or not self.run_id: return
        ...
        buf = io.BytesIO()
        np.savez(buf,
                 mask_true=mask_true, mask_pred=mask_pred,
        ...
        try:
            HfApi().upload_file(path_or_fileobj=buf, path_in_repo=path_in_repo,
    ...
    def _visualize(self, batch, outputs, state, split):
        ...
        for i in range(n):
            ...
            fig.suptitle(f'Epoch {epoch}, {split.capitalize()} Sample {sample_ids[i]}, Prob'); fig.tight_layout()
            log.setdefault(f'mask_viz/{split}/prob', []).append(wandb.Image(fig, caption=caption)); plt.close(fig)
            ...
            for t in self.thresholds:
                ...
                fig.suptitle(f'Epoch {epoch}, {split.capitalize()} Sample {sample_ids[i]}, threshold {t}'); fig.tight_layout()
                log.setdefault(f'mask_viz/{split}/thresh{t}', []).append(wandb.Image(fig, caption=caption)); plt.close(fig)
        wandb.log(log, step=state.timestamp.batch.value)


def fetch_predictions(run_id, data_dir="eturok-weizmann/good-vibrations",
    ...
    files = [f.rfilename for f in list_repo_tree(repo_id, repo_type="dataset", recursive=True)
...
def fetch_overhead_images(sample_idxs, repo_id="eturok-weizmann/vibrations"):
    ...
    ds = load_dataset(repo_id, columns=["sample_idx", "cropped_image"],
                      split="train", verification_mode="no_checks")
    return {int(row["sample_idx"]): row["cropped_image"]
            for row in ds if int(row["sample_idx"]) in sample_set}
