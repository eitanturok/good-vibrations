    ...
    for img_idx, kwargs in enumerate(pipeline_args_list):
        for sample_idx in range(args.num_validation_images):
            with autocast_ctx:
                ...
                images.append(image)
                captions.append(f"eval_image={img_idx} sample={sample_idx}: {args.validation_prompt}")
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Column in the dataset containing the SOURCE image the flow-matching trajectory starts "
        "from (edit mode) -- e.g. the empty-box reference. When set, training anchors the noised "
        "latent on this image's latent instead of pure noise, and the loss target becomes "
        "noise - (target_latent - source_latent) instead of noise - target_latent. The condition "
        "image (--cond_image_column) keeps its existing role as extra clean context tokens.",
    )
    parser.add_argument(
        "--caption_column",
        type=str,
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if args.dataset_name is not None:
            ...
            if source_image_column is not None:
                if source_image_column not in column_names:
                    raise ValueError(
                        f"`--source_image_column` value '{source_image_column}' not found in dataset columns. Dataset columns are: {', '.join(column_names)}"
                    )
                source_images = [dataset["train"][i][source_image_column] for i in range(len(dataset["train"]))]
                assert len(instance_images) == len(source_images)
        ...
        for i, img in enumerate(instance_images):
            ...
            if args.dataset_name is not None and source_images is not None:
                self.source_images.extend(itertools.repeat(source_images[i], repeats))

        def _preprocess_dest_image(dest_image):
            """Same preprocessing cond_images always used, factored out so
            source_images (edit-mode's starting-point image) can share it exactly
            rather than drift from a second hand-copied version."""
        ...
        for i, image in enumerate(self.instance_images):
            ...
            if self.cond_images:  # todo: take care of max area for buckets
                dest_image = _preprocess_dest_image(self.cond_images[i])
            source_image = None
            if self.source_images:
                source_image = _preprocess_dest_image(self.source_images[i])
            ...
            # based on the bucket assignment, define the transformations. source_image
            # (edit-mode's starting-point image) gets the SAME crop/flip as image and
            # dest_image in one call -- it must stay pixel-aligned with the target it's
            # being edited into, not just share a crop size.
            image, dest_image, source_image = self.paired_transform(
                image,
            ...
            if source_image is not None:
                self.source_pixel_values.append((source_image, bucket_idx))
    ...
    def __getitem__(self, index):
        ...
        if self.source_pixel_values:
            source_image, _ = self.source_pixel_values[index % self.num_instance_images]
    ...
    def paired_transform(self, image, dest_image=None, source_image=None, size=(224, 224), center_crop=False, random_flip=False):
        # Resize preserving aspect ratio so the image covers the bucket, then crop to the bucket size.
        # The same geometry is applied to the conditioning image (and, if given, the edit-mode source
        # image) so all three stay pixel-aligned -- source_image never gets its own independent crop.
        ...
        if source_image is not None:
            source_image = TF.resize(source_image, new_size, interpolation=transforms.InterpolationMode.BILINEAR)
        if center_crop:
            image = TF.center_crop(image, size)
            if dest_image is not None:
                dest_image = TF.center_crop(dest_image, size)
            if source_image is not None:
                source_image = TF.center_crop(source_image, size)
        else:
            ...
            if source_image is not None:
                source_image = TF.crop(source_image, i, j, h, w)
        if random_flip and random.random() < 0.5:
            image = TF.hflip(image)
            if dest_image is not None:
                dest_image = TF.hflip(dest_image)
            if source_image is not None:
                source_image = TF.hflip(source_image)
        image = TF.normalize(TF.to_tensor(image), [0.5], [0.5])
        if dest_image is not None:
            dest_image = TF.normalize(TF.to_tensor(dest_image), [0.5], [0.5])
        if source_image is not None:
            source_image = TF.normalize(TF.to_tensor(source_image), [0.5], [0.5])
        return image, dest_image, source_image
...
def collate_fn(examples):
    ...
    if any("source_images" in example for example in examples):
        source_pixel_values = [example["source_images"] for example in examples]
        ...
        batch.update({"source_pixel_values": source_pixel_values})
    return batch
            ...
            with torch.no_grad():
                if args.cache_latents:
                    with offload_models(vae, device=accelerator.device, offload=args.offload):
                        ...
                        if "source_pixel_values" in batch:
                            batch["source_pixel_values"] = batch["source_pixel_values"].to(
                                accelerator.device, non_blocking=True, dtype=vae.dtype
                            )
                            source_latents_cache.append(vae.encode(batch["source_pixel_values"]).latent_dist)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                edit_mode = args.source_image_column is not None
                if args.cache_latents:
                    model_input = latents_cache[step].mode()
                    cond_model_input = cond_latents_cache[step].mode()
                    if edit_mode:
                        source_model_input = source_latents_cache[step].mode()
                else:
                    with offload_models(vae, device=accelerator.device, offload=args.offload):
                        ...
                        if edit_mode:
                            source_pixel_values = batch["source_pixel_values"].to(
                ...
                if edit_mode:
                    source_model_input = Flux2KleinPipeline._patchify_latents(source_model_input)
                ...
                # zt = (1 - texp) * x + texp * z1
                # Edit mode: anchor the trajectory on source_model_input (the empty-box
                # latent) instead of model_input (the target), so at sigma=0 the
                # "denoised" latent is the source, not the target -- the model has to
                # actively move the latent toward the target via its predicted
                # velocity, which is what "editing" means here. See
                # Flux2KleinInpaintPipeline.prepare_latents for the equivalent
                # inference-time mechanics (scale_noise from a real image's latent
                # rather than pure noise).
                sigmas = get_sigmas(timesteps, n_dim=model_input.ndim, dtype=model_input.dtype)
                noise_anchor = source_model_input if edit_mode else model_input
                noisy_model_input = (1.0 - sigmas) * noise_anchor + sigmas * noise
                ...
                # flow matching loss. Edit mode: velocity points from the source
                # latent toward the target latent (through noise), not from the
                # target latent toward noise -- so the ODE, run from the source
                # anchor, converges on the target instead of on the source itself.
                target = noise - (model_input - source_model_input) if edit_mode else noise - model_input
