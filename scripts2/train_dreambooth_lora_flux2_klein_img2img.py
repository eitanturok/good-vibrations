...
# repo root, for utils.metrics (soft_iou/mass_error/contour_f/localization) -- the
# same suite src/model/arch.py's MaskMetric already uses, reused here rather than
# reimplemented so eval numbers are directly comparable to boombox/transformer runs.
...
from utils.metrics import LOC_KEYS, contour_f, localization, mass_error, soft_iou
...
def _pil_to_mask_tensor(img: "Image.Image", size: tuple[int, int]) -> torch.Tensor:
    """RGB PIL image -> (H,W) float tensor in [0,1] occupancy, resized to `size`
    (h,w) with NEAREST (matches src/model/dataset.py's upsample_mask: never invent
    a gradient the source didn't have). Grayscale via mean-over-channels, since the
    mask is stored as a 3-channel PNG (to_dataset() always .convert("RGB")) but is
    semantically single-channel occupancy."""
...
SEG_METRIC_KEYS = ("bce", "iou", "mass", "contour", *LOC_KEYS)


def compute_seg_metrics(pred_img: "Image.Image", true_img: "Image.Image") -> dict[str, float]:
    """Full segmentation metric suite (same keys as src/model/arch.py's SEG_KEYS) for
    one generated image against its ground truth mask. Only meaningful when the
    training target is a mask (--target mask) -- these are occupancy metrics, not
    photo-similarity metrics, so callers should skip this for --target photo runs.

    No train-step cost: only called from log_validation, which itself only runs at
    validation_epochs cadence on a small fixed set of images (2 train + 2 eval per
    the user's request) -- never inside the training loop."""
    h, w = true_img.size[1], true_img.size[0]
    true = _pil_to_mask_tensor(true_img, (h, w))
    pred = _pil_to_mask_tensor(pred_img, (h, w))
    true_b, pred_b = true.unsqueeze(0), pred.unsqueeze(0)  # metrics expect (B,H,W)

    out = {
        "bce": F.binary_cross_entropy(pred_b.clamp(1e-6, 1 - 1e-6), true_b, reduction="mean").item(),
        "iou": soft_iou(pred_b, true_b).mean().item(),
        "mass": mass_error(pred_b, true_b).mean().item(),
        "contour": contour_f(pred_b, true_b).mean().item(),
    }
    for k, v in localization(pred_b, true_b).items():
        v = v.squeeze(0)
        out[k] = float("nan") if v.isnan() else v.item()
    return out


def _side_by_side(generated: Image.Image, ground_truth: Image.Image) -> Image.Image:
    """Paste generated (left) and ground_truth (right) into one RGB canvas so they
    appear as a single wandb.Image instead of two separate gallery entries."""
    generated, ground_truth = generated.convert("RGB"), ground_truth.convert("RGB")
    h = max(generated.height, ground_truth.height)
    gen_w = round(generated.width * h / generated.height)
    gt_w = round(ground_truth.width * h / ground_truth.height)
    generated, ground_truth = generated.resize((gen_w, h)), ground_truth.resize((gt_w, h))
    sep = 4
    canvas = Image.new("RGB", (gen_w + sep + gt_w, h), (255, 255, 255))
    canvas.paste(generated, (0, 0))
    canvas.paste(ground_truth, (gen_w + sep, 0))
    return canvas
...
def log_validation(
    ...
    """`pipeline_args` is either one dict (single conditioning image, the original
    behavior) or a list of dicts (one per --validation_images entry) -- each
    generates its own image(s), all logged together under one wandb key so a
    validation step with multiple eval images shows as one gallery, not one
    overwriting the last.

    `ground_truths` (optional): list of PIL images, same length as
    pipeline_args_list, one true target per conditioning image. `split_labels`
    (optional): list of strings ("train"/"eval") for captions and metric keys.
    When compute_metrics=True and ground_truths is given, runs the full
    segmentation metric suite (compute_seg_metrics) on each generated image
    against its ground truth and logs per-image and per-split-mean scalars --
    only meaningful for --target mask runs."""
    pipeline_args_list = pipeline_args if isinstance(pipeline_args, list) else [pipeline_args]
    args.num_validation_images = args.num_validation_images if args.num_validation_images else 1
    logger.info(
        f"Running validation... \n Generating {args.num_validation_images} image(s) per conditioning image "
        f"({len(pipeline_args_list)} conditioning image(s)) with prompt: {args.validation_prompt}."
    )
    ...
    images = []
    captions = []
    metrics_by_split = {}  # split_label -> list of per-image metric dicts
    for img_idx, kwargs in enumerate(pipeline_args_list):
        split_label = split_labels[img_idx] if split_labels else None
        gt = ground_truths[img_idx] if ground_truths is not None else None
        for sample_idx in range(args.num_validation_images):
            with autocast_ctx:
                ...
                label_prefix = f"{split_label}_" if split_label else ""
                caption = f"{label_prefix}image={img_idx} sample={sample_idx}: {args.validation_prompt}"

                if compute_metrics and gt is not None:
                    m = compute_seg_metrics(image, gt)
                    caption += f" | iou={m['iou']:.3f} bce={m['bce']:.3f}"
                    metrics_by_split.setdefault(split_label or "validation", []).append(m)

                if gt is not None:
                    # paste generated + ground truth into one image so they log as a
                    # single wandb.Image, per the user's request to compare them directly.
                    images.append(_side_by_side(image, gt))
                    captions.append(f"generated | ground_truth {caption}")
                else:
                    images.append(image)
                    captions.append(f"generated {caption}")

    for tracker in accelerator.trackers:
        phase_name = "test" if is_final_validation else "validation"
        if tracker.name == "tensorboard":
            # composite generated+ground_truth images can vary in width across
            # conditioning images, so this only stacks cleanly when every logged
            # image shares one shape.
            shapes = {np.asarray(img).shape for img in images}
            if len(shapes) == 1:
                np_images = np.stack([np.asarray(img) for img in images])
                tracker.writer.add_images(phase_name, np_images, epoch, dataformats="NHWC")
            else:
                for i, img in enumerate(images):
                    tracker.writer.add_image(f"{phase_name}/{i}", np.asarray(img), epoch, dataformats="HWC")
        if tracker.name == "wandb":
            tracker.log(
                {phase_name: [wandb.Image(image, caption=cap) for image, cap in zip(images, captions)]}
            )
            for split_label, split_metrics in metrics_by_split.items():
                # mean over the (small, fixed) set of logged images for this split --
                # cheap, since split_metrics has at most num_validation_images entries.
                keys = split_metrics[0].keys()
                means = {k: float(np.nanmean([m[k] for m in split_metrics])) for k in keys}
                tracker.log({f"{split_label}/{k}": v for k, v in means.items()})
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        choices=["mask", "photo"],
        default="mask",
        help="What --image_column holds: a segmentation mask (occupancy metrics like iou/mass/contour "
        "apply) or a natural photo (they don't -- validation skips computing them). Must match how "
        "src2/data.py built the dataset.",
    )
    parser.add_argument(
        "--validation_prompt",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="comma-separated paths to multiple condition images, logged together at each validation step "
        "(one generated image per input) instead of a single repeated --validation_image.",
    )
    parser.add_argument(
        "--validation_ground_truths",
        type=str,
        default=None,
        help="comma-separated paths to the true target image for each --validation_images entry (same "
        "order, same count). When given, validation logs iou/bce/mass/contour/localization for each "
        "generated image against its ground truth -- only meaningful for --target mask runs.",
    )
    parser.add_argument(
        "--validation_split_labels",
        type=str,
        default=None,
        help="comma-separated split label ('train' or 'eval') for each --validation_images entry, same "
        "order/count -- used to caption images and to average metrics per split rather than pooling "
        "train and eval images into one number.",
    )
    parser.add_argument(
...
def main(args):
    ...
    # On-disk cache of text embeddings, keyed by (prompt, max_sequence_length). Every example
    # in this project uses the same fixed (often empty) prompt, so across runs -- and across
    # every box dataset, since the prompt never changes -- this turns "encode the prompt" into
    # a cache lookup and the Qwen3 text encoder never has to be loaded onto the GPU at all,
    # which also sidesteps the OOM from it sharing VRAM with the transformer + VAE.
    text_embed_cache_dir = Path(args.output_dir).parent / "text_embed_cache"
    text_embed_cache_dir.mkdir(parents=True, exist_ok=True)

    def _cache_key(prompt: str) -> Path:
        h = hashlib.sha256(f"{prompt}|{args.max_sequence_length}".encode()).hexdigest()
        return text_embed_cache_dir / f"{h}.pt"
    ...
    def compute_text_embeddings_cached(prompt, text_encoding_pipeline):
        """Like compute_text_embeddings, but checks the on-disk cache BEFORE moving the
        text encoder onto the GPU (via offload_models), not just before running its forward
        pass -- offload_models.__enter__ does the `.to(device)` unconditionally, so checking
        the cache only inside compute_text_embeddings would still OOM on a hit. Run
        src2/precompute_text_embeds.py ahead of time so this is always a hit during training.

        `prompt` is either a single string (the fixed --instance_prompt path) or a list of
        strings (one per sample in a batch, from the caption_column path) -- cache per string,
        not per list, so this matches the per-prompt files precompute_text_embeds.py writes
        and still hits when a batch mixes prompts that are each individually cached."""
        prompts = [prompt] if isinstance(prompt, str) else list(prompt)
        cache_paths = [_cache_key(p) for p in prompts]
        if all(p.exists() for p in cache_paths):
            cached = [torch.load(p, map_location=accelerator.device) for p in cache_paths]
            prompt_embeds = torch.cat([c["prompt_embeds"] for c in cached], dim=0)
            text_ids = cached[0]["text_ids"]  # text_ids don't depend on prompt content
            return prompt_embeds, text_ids
        with offload_models(text_encoding_pipeline, device=accelerator.device, offload=args.offload):
            prompt_embeds, text_ids = compute_text_embeddings(prompts, text_encoding_pipeline)
        for i, p in enumerate(prompts):
            torch.save({"prompt_embeds": prompt_embeds[i : i + 1].cpu(), "text_ids": text_ids.cpu()}, cache_paths[i])
        return prompt_embeds, text_ids
    ...
    if not train_dataset.custom_instance_prompts:
        instance_prompt_hidden_states, instance_text_ids = compute_text_embeddings_cached(
            args.instance_prompt, text_encoding_pipeline
        )

    if args.validation_prompt is not None:
        validation_image_paths = (
            [p.strip() for p in args.validation_images.split(",")] if args.validation_images else [args.validation_image]
        )
        shared_prompt_embeds, _text_ids = compute_text_embeddings_cached(args.validation_prompt, text_encoding_pipeline)
        shared_negative_prompt_embeds, _text_ids = compute_text_embeddings_cached("", text_encoding_pipeline)
        # list of per-image kwargs, so log_validation can be called once per validation image;
        # validation_kwargs (singular) kept as the first entry for backward compat with any
        # single-image code path below that still reads it directly.
        validation_kwargs_list = [
            {
                "image": load_image(p).convert("RGB"),
                "prompt_embeds": shared_prompt_embeds,
                "negative_prompt_embeds": shared_negative_prompt_embeds,
            }
            for p in validation_image_paths
        ]
        validation_kwargs = validation_kwargs_list[0]

        # Ground truths + split labels, same order/count as validation_image_paths.
        # Loaded once here (not per validation call) since these are the SAME fixed
        # images every epoch, per the user's request.
        validation_ground_truths = (
            [load_image(p).convert("RGB") for p in args.validation_ground_truths.split(",")]
            if args.validation_ground_truths
            else None
        )
        validation_split_labels = (
            [s.strip() for s in args.validation_split_labels.split(",")] if args.validation_split_labels else None
        )
        compute_val_metrics = validation_ground_truths is not None and args.target == "mask"
            ...
            with torch.no_grad():
                ...
                if train_dataset.custom_instance_prompts:
                    if args.fsdp_text_encoder:
                        ...
                        # cached variant: every row in this project's dataset shares the same
                        # fixed prompt, so after the first batch (or after
                        # src2/precompute_text_embeds.py has pre-seeded the cache) this never
                        # touches the GPU with the text encoder again.
                        prompt_embeds, text_ids = compute_text_embeddings_cached(
    ...
    # Log a baseline validation pass on the UNTRAINED model before any optimizer
    # step, so the wandb gallery has a true epoch-0 reference to compare later
    # epochs against. Skipped when resuming from a checkpoint (initial_global_step
    # > 0) -- there's no meaningful "before any training" state to log there.
    if accelerator.is_main_process and args.validation_prompt is not None and initial_global_step == 0:
        ...
        log_validation(
            ...
            args=args,
            accelerator=accelerator,
            pipeline_args=validation_kwargs_list,
            epoch=0,
            torch_dtype=weight_dtype,
            ground_truths=validation_ground_truths,
            split_labels=validation_split_labels,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            logs = {
                ...
                "step_time": time.perf_counter() - step_start_time,  # this step only, not cumulative
            }
